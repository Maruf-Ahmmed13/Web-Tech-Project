...
// --- HANDLE DELETE LOGIC (Reverse the Stock Addition) ---
if (isset($_GET['delete'])) {
    ...
    if($check->num_rows > 0) {
        ...
        if ($status === 'Received') {
            ...
            // 2. Remove Stock (Since we are cancelling the purchase)
            $details = $conn->query("SELECT product_id, quantity FROM purchase_order_details WHERE purchase_order_id = $po_id");
            while ($item = $details->fetch_assoc()) {
                $pid = $item['product_id'];
                $qty = $item['quantity'];
                // SUBTRACT because we are deleting the supply
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                ...
                                <a href="purchase_orders.php?delete=<?php echo $row['id']; ?>" class="action-btn delete-btn" 
                                   onclick="return confirm('Are you sure? This will remove the added stock.');">
